...
**CampusPulse** is a production-minded campus platform for students, faculty, club coordinators, and administrators. It centralizes campus issue reporting, engagement, and institutional communication.
...
- Instant account creation with automatic sign-in, plus password-reset flows. Local development uses a console mail transport; production delivery is an isolated provider boundary.
...
The command makes that account a `SUPER_ADMIN` and is idempotent.

### Local email flow

With `MAIL_MODE=console`, password-reset URLs are written to the API terminal. Set `MAIL_MODE=smtp` only after supplying a real email transport implementation in `apps/api/src/services/email.service.ts`.
...
| --- | --- | --- |
| POST | `/api/auth/register` | Create a student account and sign in immediately |
